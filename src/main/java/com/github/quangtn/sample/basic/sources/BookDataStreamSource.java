package com.github.quangtn.sample.basic.sources;
...
public final class BookDataStreamSource implements Function<StreamExecutionEnvironment, DataStreamSource<String>> {
    ...
    public DataStreamSource<String> apply(StreamExecutionEnvironment env) {
        ...
        return env.fromSource(FileSource.forRecordStreamFormat(new TextLineInputFormat(), paths.toArray(new Path[0])).build(),
                WatermarkStrategy.noWatermarks(), "read source");
    }

    static Collection<Path> scan(Path... paths) {
        PathScanner pathScanner = new PathScanner(new FileExtensionFilter(".json.gz"));
        try {
