...
public class BookJdbcSink implements Function<DataStream<Book>, DataStreamSink<Book>> {
    ...
    public static void accept(PreparedStatement s, Book b) throws SQLException {
        ...
        s.setDate(7, b.dateFirstAvailable != null ? Date.valueOf(b.dateFirstAvailable): null);
        ...
        s.setObject(34, asJsonString(b.bestSellerRank));
    }

    @Override
    public DataStreamSink<Book> apply(DataStream<Book> in) {
        return in.addSink(sink(executionOptions, connectionOptions)).name("persist to storage");
    }

    static SinkFunction<Book> sink(JdbcExecutionOptions executionOptions,
                                   JdbcConnectionOptions connectionOptions) {
