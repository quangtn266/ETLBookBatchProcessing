...
import com.github.quangtn.sample.shared.jdbc.PreparedStatementCommand;
...
public final class AddIngestionLogEntryJdbcOperator
        ...
        implements Function<DataStream<Path>, SingleOutputStreamOperator<IngestionSource<Path>>> {

    private static final Logger LOGGER = LogManager.getLogger();

    private static final RetryOptions RETRY_OPTIONS = RetryOptions.builder()
            .max(5).backoff(ExpBackoff.builder().initial(Duration.ofSeconds(1))
                    .multiplier(1.5).factor(0.25).build()).build();
    ...
    public SingleOutputStreamOperator<IngestionSource<Path>> apply(
            DataStream<Path> in ) {
        return in.map(this).setParallelism(1).name("add ingestion log entry");
    }
        ...
        return RetryThrowingSupplier.get(
                ...
                (r, ex) -> {
                    ...
                    if (retry) {
                        ...
                        if (preparedStatementCommand != null && !preparedStatementCommand.isValid()) {
